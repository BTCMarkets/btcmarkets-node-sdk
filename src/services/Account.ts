...
export default class Account extends Http {
    public async getTradingFees(): Promise<ITradingFeesResponse> {
        let response = await this.privateRequest(HttpMethods.GET, '/accounts/me/trading-fees', null, null);
        return getParsedBody(response) as ITradingFeesResponse;
    }
    public async getWithdrawallimits(): Promise<any> {
        return await this.privateRequest(HttpMethods.GET, '/accounts/me/withdrawal-limits', null, null);
    }
    public async getBalances(): Promise<IBalancesResponse> {
        let response = await this.privateRequest(HttpMethods.GET, '/accounts/me/balances', null, null);
    ...
    public async getTransactions(queryParmas: ITransactionsRequest): Promise<ITransactionsResponse> {
        let response = await this.privateRequest(HttpMethods.GET, '/accounts/me/transactions', queryParmas, null);
